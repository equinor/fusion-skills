...
When the question is mainly about framework package ownership, exact API behavior, or finding a supporting example, use the companion skill `fusion-reasearch-framework` first and then apply that evidence during the integration review.
...
If the runtime supports companion skills, follow the `fusion-reasearch-framework` workflow for evidence gathering: choose the right search lane, capture `metadata.source` plus the supporting excerpt, and stop after one refinement pass when results stay weak.
...
5. If the review is blocked on uncertain framework behavior, gather source-backed evidence via `fusion-reasearch-framework` before deciding whether the code is correct.
